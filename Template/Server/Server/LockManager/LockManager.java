				...
				synchronized (this.lockTable) {
					...
					if (!bConflict) {
						...
						if (bConvert.get(0) == true) {
							...
							//Convert Shared lock into an exclusive lock
							TransactionLockObject xLockObject1 = new TransactionLockObject(xid, data, TransactionLockObject.LockType.LOCK_READ);
							DataLockObject dataLockObject1 = new DataLockObject(xid, data, TransactionLockObject.LockType.LOCK_READ);

							this.lockTable.remove(xLockObject1);
		...
		for (int i = 0; i < size; i++)
		{
			...
			{    
				...
				else if (dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_WRITE)
				{
					...
					//TODO: Lock conversion
					//Case 1: transaction already had a READ lock
